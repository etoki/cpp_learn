...
main(int argc, char* argv[])
{
    try
    {
        ...
        status=clEnqueueNDRangeKernel(queue, kernelMin, 1, NULL,
                                    globalSize, 0, 0, NULL, &eventKernel[0]);
        if(status!=CL_SUCCESS)
            throw Cexp("clEnqueueTask failed.");

        printf("        enqueued clEnqueueNDRangeKernel API(min).\n");


                                                    // request execute kernel
        status=clEnqueueNDRangeKernel(queue, KernelMax, 1, NULL,
                                    globalSize, 0, 0, NULL, &eventKernel[1]);
        if(status!=CL_SUCCESS)
            throw Cexp("clEnqueueTask failed.");

        printf("        enqueued clEnqueueNDRangeKernel API(max).\n");
