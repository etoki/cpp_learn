...
#include "filter.op"            // any size operator, but  must odd w/h
...
filterCL(const char* clName, const Cbmp* bmp)
{
    ...
    param.height    =bmp->mAbsHeight;
    param.width     =bmp->mPdib->biWidth;
    param.rowPitch  =bmp->mRowPitch;
    param.pixelPitch=bmp->mPixelPitch;
    param.opHeight  =sizeof(filterOp)/sizeof(filterOp[0]);
    param.opWidth   =sizeof(filterOp[0])/sizeof(filterOp[0][0]);
    ...
    cl_mem in=clUtils.createInBuffer(bmp->mImageSize, bmp->mPbitmap);
                                                    ...
                                                    // mem obj(out)
    cl_mem out=clUtils.createOutBuffer(bmp->mImageSize);
    ...
    size_t globalSize[]={param.width-param.opWidth+1,
                                param.height-param.opHeight+1};
