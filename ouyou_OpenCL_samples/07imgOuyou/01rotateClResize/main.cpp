...
affineTransCL(const char* clName, Cbmp bmp[], const float inDegree)
{
    ...
    status+=clSetKernelArg(kernel, 8, sizeof(cl_float), (void *)&degree);
    status+=clSetKernelArg(kernel, 9, sizeof(cl_mem),   (void *)&in);
    status+=clSetKernelArg(kernel,10, sizeof(cl_mem),   (void *)&out);
    if(status!=0)
