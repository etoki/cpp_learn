...
Cbmp::loadFromFile(const char* bmpFName)
{
    ...
    if((fp=fopen(bmpFName, "rb"))==0)               // open bitmap file
        throw Cexp("input file open failed.");

    if(stat(bmpFName, &statbuf)!=0)                 // for SIDBA
        throw Cexp("function stat() failed.");      // for SIDBA

    if(readHeader(fp)!=0)                           // read file header
    {
        fclose(fp);
        throw Cexp("failed to read bitmap file header.");
    }
    ...
    if(readDib(fp)!=0)                              // read dib
    {
        SAFE_FREE(mPdib);
        fclose(fp);
        throw Cexp("failed to read bitmap file body.");
    }
    ...
    if((fp=fopen(bmpFName, "wb"))!=0)               // open file
    {
        if(writeHeader(fp)==0)                      // write header
        {
            if(writeDib(fp)!=0)                     // write dib
                throw Cexp("failed to write dib.");
        }
        else
            throw Cexp("failed to write header.");
    }
    else
        throw Cexp("failed to open file.");
