...
search(const Cbmp bmp[])
{
    ...
    printf("        enqueued clEnqueueNDRangeKernel API.\n");

    status=clEnqueueBarrierWithWaitList(queue, 0, NULL, NULL);
    if(status!=CL_SUCCESS)
        throw Cexp("clEnqueueBarrierWithWaitList failed.");

    printf("        enqueued clEnqueueBarrierWithWaitList API.\n");

    cl_event eventRead;
    status=clEnqueueReadBuffer(queue, memSum, CL_FALSE, 0,
                        sizeof(cl_int)*(param.sumArrayHeight
    ...
    printf("        enqueued clEnqueueReadBuffer API.\n");

    status=clWaitForEvents(1, &eventRead);
    if(status!=CL_SUCCESS)
